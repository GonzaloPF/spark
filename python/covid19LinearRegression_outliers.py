...
def linearReg(x):
    ...
    casos = []
    caso_old = 0
    #calculate the cases per day, not the acumulated ones
    for item in data:
        code.append(x[0])
        date.append(float(dateToUnix(item[0])))
        
        if not caso_old == 0:
            caso = float(item[1]) - caso_old
            caso_old = float(item[1])
            casos.append(caso)
        else:
    ...
    x_pred = np.array(date).reshape(-1, 1)
    y_pred = np.array(casos).reshape(-1, 1)
    #calculate outliers
    z = np.abs(stats.zscore(y_pred))
    
    outliers = np.where(z > 1)
    
    delete = [num for num in outliers[0]]
    #remove outliers
    y_pred_new = np.delete(y_pred, delete, axis=0).reshape(1, -1).tolist()[0]
    x_pred_new = np.delete(x_pred, delete, axis=0).reshape(1, -1).tolist()[0]
    
    #create the model with the values
    model = np.polyfit(x_pred_new, y_pred_new, 1)
...
df_casos = spark.read.load("dataset/covid19casos.csv", format="csv", sep=",", header="True", emptyValue='')
...
end = end.withColumn('last_day_total', end["_3"].cast("float"))
